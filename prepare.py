...
def draw_partial_dependence_2d(model, x_train, config, save=False):
    pdp = partial_dependence(model, x_train, features=config['features'], kind='average',
...
def draw_scatter(train_true, train_pred,
    ...
    train_true = np.array(train_true).ravel()
    ...
    if test_lower is not None:
        test_lower = np.array(test_lower).ravel()
    if test_upper is not None:
        test_upper = np.array(test_upper).ravel()
    font_common = {
    ...
    # ===== 绘制散点 + 回归线 =====
    sns.regplot(x=test_true, y=test_pred, color="orange", label="test", scatter_kws={'s': 50, 'alpha': 0.7},ci = None)
    ...
    # 设置坐标轴
    ax = plt.gca()
    ax.tick_params(axis='both', which='major', direction='out', length=8, width=2, color='black', labelsize=14,
